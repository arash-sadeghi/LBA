...
def plotter(pop,indx):
    ...
    for v in Times:
        ...
        for i in range(lst_av_sh[0]):
            ...
            Y3.append(Q3)
        linestyle='-'
        Etol='LBA $\sigma$ = 30 $\degree$'
        if indx==0:
        #     Y=Y*len(X)
        #     Y1=Y1*len(X)
        #     Y3=Y3*len(X)
            Etol='LBA $\sigma$ = 60 $\degree$'
            linestyle='--'
            marker='^'
        if indx==2:
            # Y=Y*len(X)
            # Y1=Y1*len(X)
            # Y3=Y3*len(X)
            Etol='LBA $\sigma$ = 0 $\degree$'
            linestyle='-.'
            marker='s'
        if indx==1:
            marker='o'
        ax[ii].plot(X,Y,label=Etol,color=pltt[indx],linewidth=3,linestyle=linestyle,marker=marker,markersize=12)
...
ax[0].legend(fontsize=30,ncol=3)
