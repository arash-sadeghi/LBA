...
def plotter(pop,indx):
    ...
    for v in Times:
        ...
        ax[axindx].set_xlim(X[0],X[-1])
        ax[axindx].set_ylim(0,1)
        ii+=1
    ax[axindx].set_xticks(X,minor=False)
    # ax[1].set_xticks(X,minor=False)
    # ax[2].set_xticks(X,minor=False)

    ax[axindx].set_xticklabels(X,fontsize=30,fontweight='bold')
...
ax[0].set_xlabel('Population size',fontsize=30,fontweight='bold')
ax[0].legend(fontsize=30,ncol=2)
os.chdir("..")
# plt.savefig("/home/arash/Desktop/performance_vs_Etol"+ctime(time()).replace(':','_')+".pdf")
plt.savefig("/home/arash/Desktop/POP size.pdf")
